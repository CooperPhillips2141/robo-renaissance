...
def sound_mix(emotions, percentages, target_loudness = -30.0):
    ...
    for i in range(len(emotions)):
        if percentages[i] != 0:
             # Load the audio file
             audio = AudioSegment.from_file("Balanced_Audio/" + emotions[i])
             ...
             mixed_audio = mixed_audio + adjusted_audio + adjusted_audio
...
emotions = np.array(["Happy Balanced.wav", "Sad Balanced.mp3", "Disgusted Balanced.mp3", "Anger Balanced.mp3", "Fear Balanced.mp3", "Bad Balanced.mp3", "Surprised Balanced.wav"]) # Variable to store past percentages list
percentages_past = 0
